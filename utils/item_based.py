...
def get_item_based_reccomendation(user_profile, datasets_path='./data/',top_k:int =30, test:bool=False) -> pd.DataFrame:
    ...
    if test:
        return transformed_db.apply(lambda x: distance.cosine(x.values, mean_dist), axis='columns').values
