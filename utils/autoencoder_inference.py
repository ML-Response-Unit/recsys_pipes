...
class AE(nn.Module):
    ...
    def forward(self, features):
        # encoder
        x = torch.relu(self.encoder_hidden_layer_1(features))
        x = torch.relu(self.encoder_hidden_layer_2(x))
        x = torch.relu(self.encoder_output_layer(x))

        # decoder
        x = torch.relu(self.decoder_hidden_layer_1(x))
        x = torch.relu(self.decoder_hidden_layer_2(x))
        x = torch.relu(self.decoder_output_layer(x))
        return x
class AEPredictor:
